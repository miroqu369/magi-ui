...
function App() {
  ...
  // 設定
  const [magiSysUrl, setMagiSysUrl] = useState('/proxy/sys');
  const [magiAcUrl, setMagiAcUrl] = useState('/proxy/ac');
            ...
            {stockResults[symbol] && (
              <div className="card-body">
                {stockResults[symbol].error ? (
                  ...
                  <>
                    <div className={`recommendation ${stockResults[symbol]?.aiConsensus?.consensus?.recommendation?.toLowerCase()}`}>
                      {stockResults[symbol]?.aiConsensus?.consensus?.recommendation || 'N/A'}
                    </div>
                    <div className="confidence">
                      信頼度: {stockResults[symbol]?.aiConsensus?.consensus?.confidence || 0}%
                    </div>
                    {stockResults[symbol]?.aiConsensus?.consensus?.votes && (
                      <div className="votes">
                        BUY: {stockResults[symbol]?.aiConsensus?.consensus?.votes.BUY || 0} / 
                        HOLD: {stockResults[symbol]?.aiConsensus?.consensus?.votes.HOLD || 0} / 
                        SELL: {stockResults[symbol]?.aiConsensus?.consensus?.votes.SELL || 0}
                      </div>
                    )}
                  </>
                )}
